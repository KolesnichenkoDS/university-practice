...
void drawAxes(Graph *graph) {
  ...
  for (double x = 0, xx = 0; xx < graph->sizeX; x += graph->unitX, xx += graph->unitSizeX) {
    ...
    if (x1 >= 0) {
      graph->buffer[graph->centerY][(int) x1] = '+';
      char xs[100];
      xs[0] = '\0';
      sprintf(xs, " %.2f", -x);
      if ((int) x1 > strlen(xs) / 2 &&
        ...
        for (int i = 0; i < strlen(xs); i++) {
          graph->buffer[graph->centerY - 1][(int) x1 + i - strlen(xs) / 2] = xs[i];
        }
      }
    }
    if (x2 < graph->sizeX) {
  ...
  for (double y = 0, yy = 0; yy < graph->sizeY; y += graph->unitY, yy += graph->unitSizeY) {
    ...
    if (y1 >= 0) {
      graph->buffer[(int) y1][graph->centerX] = '+';
      char ys[100];
      ys[0] = '\0';
      sprintf(ys, " %.2f", y);
      if ((int) y1 != graph->centerY) {
        for (int i = 0; i < strlen(ys); i++) {
          graph->buffer[(int) y1][graph->centerX + 1 + i] = ys[i];
        }
      }
    }
    if (y2 < graph->sizeY) {
      graph->buffer[(int) y2][graph->centerX] = '+';
      char ys[100];
      ys[0] = '\0';
      sprintf(ys, " %.2f", -y);
      if ((int) y2 != graph->centerY) {
        for (int i = 0; i < strlen(ys); i++) {
          graph->buffer[(int) y2][graph->centerX + 1 + i] = ys[i];
        }
      }
    }
  }

  graph->buffer[0][graph->centerX] = '^';
  graph->buffer[graph->centerY][graph->sizeX - 1] = '>';
}
...
void drawAnnotations(Graph *graph) {
  ...
  char *ys = "Y axis";
  for (int i = 0; i < strlen(xs); i++) {
    graph->buffer[graph->centerY + 2][graph->sizeX - strlen(xs) + i] = xs[i];
  }
  for (int i = 0; i < strlen(ys); i++) {
    graph->buffer[0][graph->centerX - strlen(ys) + i - 1] = ys[i];
  }

  char *fs = "{ y = x + cos(x) }";
  for (int i = 0; i < strlen(fs); i++) {
    graph->buffer[graph->sizeY - 1][graph->sizeX - strlen(fs) + i] = fs[i];
  }
}
...
void drawPoint(Graph *graph, double x, int xPos) {
  ...
  if (yPos >= 0 && yPos < graph->sizeY) {
    char c = graph->buffer[yPos][xPos];
    if (c == ' ' || c == '-' || c == '!' || c == '+')
      graph->buffer[yPos][xPos] = '*';
  }
}
